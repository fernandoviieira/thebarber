    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 px-4 md:px-0">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {filtered.map(c => (
            <div key={c.id} className="bg-slate-900/40 border border-white/5 rounded-3xl md:rounded-[2.5rem] p-6 md:p-8 backdrop-blur-md hover:border-amber-500/30 transition-all flex flex-col h-full group relative overflow-hidden">

              <div className="flex justify-between items-start mb-5 md:mb-6">
                <div className="flex gap-2 md:gap-3">
                  <div className="w-11 h-11 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 shrink-0">
                  ...
                  <button
                    ...
                    className="w-11 h-11 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-blue-500/5 hover:bg-blue-500/20 active:bg-blue-500/20 flex items-center justify-center text-blue-500/40 hover:text-blue-500 active:text-blue-500 transition-all opacity-0 group-hover:opacity-100 md:opacity-0 shrink-0"
                  ...
                  <button
                    ...
                    className="w-11 h-11 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-red-500/5 hover:bg-red-500/20 active:bg-red-500/20 flex items-center justify-center text-red-500/40 hover:text-red-500 active:text-red-500 transition-all opacity-0 group-hover:opacity-100 md:opacity-0 shrink-0"
                ...
                <div className="flex flex-col items-end gap-2">
                  {isBirthdayToday(c.birth_date) && (
                    <button
                      ...
                      className="bg-pink-500/10 text-pink-500 hover:bg-pink-500 hover:text-white active:bg-pink-500 active:text-white px-2.5 md:px-3 py-1 rounded-full text-[7px] md:text-[8px] font-black uppercase border border-pink-500/20 transition-all flex items-center gap-1 whitespace-nowrap animate-bounce"
                      title="Enviar mensagem de aniversário"
                    >
                      <Cake size={9} className="md:w-[10px] md:h-[10px]" /> Aniversário
                    </button>
                  )}
                  {c.customer_packages?.some((p: any) => p.used_credits < p.total_credits) ? (
                    <div className="bg-green-500/10 text-green-500 px-2.5 md:px-3 py-1 rounded-full text-[7px] md:text-[8px] font-black uppercase border border-green-500/20 whitespace-nowrap">
                      Pacote Ativo
                    </div>
                  ) : (
                    <button
                      onClick={() => { setTargetCustomer(c); setIsAddPackageModalOpen(true); }}
                      className="bg-amber-500/10 text-amber-500 hover:bg-amber-500 hover:text-black active:bg-amber-500 active:text-black px-2.5 md:px-3 py-1 rounded-full text-[7px] md:text-[8px] font-black uppercase border border-amber-500/20 transition-all flex items-center gap-1 whitespace-nowrap"
                    >
                      <PackagePlus size={9} className="md:w-[10px] md:h-[10px]" /> Novo Pacote
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="flex-1">
                <h4 className="text-lg md:text-xl font-black text-white uppercase italic tracking-tighter mb-1 break-words">
                  {c.name}
                </h4>
                <div className="mb-5 md:mb-6 space-y-1">
                  <p className="text-slate-500 text-[11px] md:text-xs font-bold flex items-center gap-2">
                    <Phone size={11} className="text-amber-500 shrink-0 md:w-3 md:h-3" />
                    <span className="break-all">{c.phone}</span>
                  </p>

                  {c.birth_date && (
                    <p className="text-amber-500/80 text-[10px] font-black uppercase italic flex items-center gap-2">
                ...
                <div className="space-y-3">
                  {c.customer_packages?.map((pkg: any) => (
                    <div key={pkg.id} className="bg-black/30 rounded-2xl md:rounded-3xl p-4 md:p-5 border border-white/5 space-y-3 md:space-y-4 shadow-inner">
                      <div className="flex justify-between items-center gap-2">
                        <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest truncate flex-1">
                          {pkg.package_name}
                        </p>
                        <div className="flex gap-2 shrink-0">
                          <button
                            onClick={() => { setEditingPkgId(pkg.id); setTempCredits(pkg.total_credits) }}
                            className="text-slate-600 hover:text-amber-500 active:text-amber-500 transition-colors p-1"
                          >
                            <Edit3 size={13} className="md:w-[14px] md:h-[14px]" />
                          </button>
                          <button
                            onClick={() => handleDeletePackage(pkg.id)}
                            className="text-slate-600 hover:text-red-500 active:text-red-500 transition-colors p-1"
                          >
                      ...
                      <div className="flex flex-wrap gap-1.5 md:gap-2">
                        {[...Array(Number(editingPkgId === pkg.id ? tempCredits : pkg.total_credits))].map((_, i) => (
                          <div
                            key={i}
                            className={`h-2.5 md:h-3 flex-1 min-w-[18px] md:min-w-[20px] rounded-full ${i < pkg.used_credits
                              ? 'bg-slate-800'
                      ...
                      <div className="flex justify-between items-center px-1">
                        <span className="text-[8px] md:text-[9px] font-black text-amber-500/50 uppercase italic">
                          Restam {pkg.total_credits - pkg.used_credits}
                        </span>
                      </div>
                      {editingPkgId === pkg.id && (
                        <div className="flex items-center justify-between bg-white/5 p-2 rounded-xl mt-2 animate-in slide-in-from-top-2">
                          <input
                            type="number"
                            value={tempCredits}
                            onChange={(e) => setTempCredits(Number(e.target.value))}
                            className="bg-transparent text-amber-500 font-black text-xs w-12 outline-none px-2"
                          />
                          <button
                            onClick={() => handleUpdateCredits(pkg.id)}
                            className="bg-amber-500 hover:bg-amber-400 active:bg-amber-400 text-black px-3 py-1 rounded-lg text-[8px] font-black uppercase transition-all active:scale-95"
                          >
                            Salvar
                          </button>
                        </div>
