    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div key={c.id} className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-md hover:border-amber-500/30 transition-all flex flex-col h-full group relative overflow-hidden">
                ...
                <div className="space-y-3">
                    ...
                    <div key={pkg.id} className="bg-black/30 rounded-3xl p-5 border border-white/5 space-y-4 shadow-inner">
                        ...
                        {[...Array(Number(editingPkgId === pkg.id ? tempCredits : pkg.total_credits))].map((_, i) => (
                            <div key={i} className={`h-3 flex-1 min-w-[20px] rounded-full ${i < pkg.used_credits ? 'bg-slate-800' : 'bg-gradient-to-r from-amber-400 to-amber-600 shadow-[0_0_10px_rgba(245,158,11,0.2)]'}`} />
                        ))}
                        </div>
                        <div className="flex justify-between items-center px-1">
                            <span className="text-[9px] font-black text-amber-500/50 uppercase italic">Restam {pkg.total_credits - pkg.used_credits}</span>
                        </div>
                        {editingPkgId === pkg.id && (
                        <div className="flex items-center justify-between bg-white/5 p-2 rounded-xl mt-2 animate-in slide-in-from-top-2">
                            <input type="number" value={tempCredits} onChange={(e) => setTempCredits(Number(e.target.value))} className="bg-transparent text-amber-500 font-black text-xs w-12 outline-none px-2" />
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm">
          <div className="bg-[#121418] border border-white/10 w-full max-w-lg rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center border-b border-white/5 pb-6 mb-8">
              ...
              <button onClick={() => setIsAddPackageModalOpen(false)} className="text-slate-500 hover:text-white transition-colors"><X size={32} /></button>
            </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm">
          <div className="bg-[#121418] border border-white/10 w-full max-w-xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center border-b border-white/5 pb-6 mb-8">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-500 hover:text-white transition-colors"><X size={32} /></button>
            </div>
            <form onSubmit={handleCreateCustomer} className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <input required placeholder="Nome" value={newCustomer.name} onChange={e => setNewCustomer({...newCustomer, name: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-2xl p-5 text-white font-bold outline-none focus:border-amber-500" />
                <input required placeholder="WhatsApp" value={newCustomer.phone} onChange={e => setNewCustomer({...newCustomer, phone: e.target.value})} className="w-full bg-slate-900 border border-white/10 rounded-2xl p-5 text-white font-bold outline-none focus:border-amber-500" />
              </div>
              ...
              {newCustomer.hasPackage && (
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                  <input placeholder="Nome do Pacote" value={newCustomer.packageName} onChange={e => setNewCustomer({...newCustomer, packageName: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl p-4 text-white text-xs font-bold outline-none focus:border-amber-500" />
                  <div className="grid grid-cols-2 gap-4">
                    <input type="number" placeholder="Cortes" value={newCustomer.total_credits} onChange={e => setNewCustomer({...newCustomer, total_credits: e.target.value})} className="bg-slate-950 border border-white/10 rounded-xl p-4 text-white text-xs font-bold outline-none focus:border-amber-500" />
                    <input type="number" placeholder="Valor R$" value={newCustomer.price_paid} onChange={e => setNewCustomer({...newCustomer, price_paid: e.target.value})} className="bg-slate-950 border border-white/10 rounded-xl p-4 text-white text-xs font-bold outline-none focus:border-amber-500" />
                  </div>
                </div>
              )}
              <button type="submit" className="w-full bg-amber-500 hover:bg-amber-400 text-black py-6 rounded-[2rem] font-black uppercase text-xs tracking-widest transition-all shadow-xl active:scale-95">Cadastrar Cliente</button>
            </form>
          </div>
        </div>
      )}
